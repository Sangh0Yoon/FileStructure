...
{
	Clear ();
	// int값 하나를 읽어서 BufferSize에 저장.
	stream . read ((char*)&BufferSize, sizeof(BufferSize));//extract the record size
	if (stream.fail()) return FALSE;
	if (BufferSize > MaxBytes) return FALSE; // buffer overflow
	stream . read (Buffer, BufferSize);//read the proper number of bytes into the buffer
	return stream . good ();
}
...
int DelimTextBuffer :: Pack (const char * str, int size) //copy str to buffer
...
// if size = -1 (default) use strlen(str) as Delim of field
{
	...
	memcpy (&Buffer[start], str, len); // copy the value of memory
	Buffer [start+len] = Delim; // add delimeter
	BufferSize = NextByte;// trace the current position in the buffer
	return TRUE;
}
...
// extract the value of the next field of the buffer
{
	...
	// NextByte는 read에 clear()에 의해서 0이 되었음.
	// BufferSize는 read에 의해서 한 레코드길이만큼 저장되어 있음.
...
int DelimTextBuffer::Unpack(int& n)
// extract the value of the next field of the buffer
{
	int len = -1; // length of packed string
	// NextByte는 read에 clear()에 의해서 0이 되었음.
